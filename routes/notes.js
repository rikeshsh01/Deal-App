...
router.get('/singlepost/:postid', fetchuser, async (req, res) => {
  try {
    ...
    // Fetch single post
    const singlePost = await Notes.findOne({ _id: postId });
    ...
    const [comments, users, additionalDetails, tagDetails, subtagDetails] = await Promise.all([
      Comments.find({ noteId: postId }),
      Users.find(),
      AdditionalDetails.find({ noteId: postId }),
      Tag.find({ _id: singlePost.tagId }),
      SubTag.find({ _id: singlePost.subtagId })
    ]);
    ...
    // Add user details and related data to the single post
    const noteUserDetails = users.find(user => user._id.toString() === singlePost.userId.toString());
